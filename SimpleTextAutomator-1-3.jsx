...
var palette = new Window("palette"); 
    ...
    var trace = palette.add("statictext", undefined, undefined, {name: "trace"}); 
    trace.text = "StaticText"; 
    trace.preferredSize.width = 440; 
    trace.preferredSize.height = 50; 
...
var statictext2 = group3.add("statictext", undefined, undefined, {name: "statictext2"}); 
    statictext2.text = "Data:"; 

var grPrevData = group3.add("group", undefined, {name: "grPrevData"}); 

var dataBox_array = []; 
var dataBox = grPrevData.add("listbox", undefined, undefined, {name: "dataBox", items: dataBox_array, showHeaders: true}); 
    dataBox.preferredSize.width = 440; 
    dataBox.preferredSize.height = 200;
var previewList = dataBox;
...
getFileButton.onClick = function() {
    ...
    if (file) {
		...
		//updatePanel();
		fileData = readCSV();
        //fillDataBox(fileData);
        //fillDataBox()
        setList(fileData,previewList)
...
function setList(arr,obj){

    grPrevData.remove(obj)
    trace.text = arr[0].split(",").length
    
    var dataBox = grPrevData.add("listbox", undefined, undefined, {
        name: "dataBox", 
        numberOfColumns: arr[0].split(",").length, 
        showHeaders: true,
        columnTitles: arr[0].split(",")});
        dataBox.preferredSize.width = 440; 
        dataBox.preferredSize.height = 200;
        addListItems(dataBox,arr);
        previewList = dataBox;

}

function addListItems(obj,arr){
    dataBox_array = [];
     for(var i = 1; i< arr.length;i++){
        dataBox_array.push(fileData[j].split(","))
        with (obj.add ("item",arr[i].split(",")[0])){
            for(var j = 1; j < arr[i].split(",").length;j++){
                subItems[j-1].text = arr[i].split(",")[j];
            }
...
function fillDataBox() {
    ...
    dataBox.numberOfColumns = dataBox_array[0].length;
    dataBox.showHeaders =  true;
    dataBox.columnTitles = dataBox_array[0];
    

   for (var i=0; i<dataBox_array.length;i++ ){
        dataBox.add("item",dataBox_array[i]);
    }
...
function getComp(str){
    var comp;
    for (var i = 1; i <= app.project.numItems; i ++) {
        if ((app.project.item(i) instanceof CompItem) && (app.project.item(i).name === str)) {
            if (comp) {
